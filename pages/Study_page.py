...
class StuDy:
    ...
    # Nếu là "Lớp 9", "Lớp 12"... lấy số để build id input
        ...
        if num:
            cb   = self.page.locator(f"input#select-grade-{num}")                  # input bị ẩn
            lbl  = self.page.locator(f"label[for='select-grade-{num}']")           # label hiển thị
            if cb.count() and not cb.is_checked():                                  # ❗ chỉ click nếu chưa chọn
        ...
        if lbl.count():
            ...
            if for_id:
                ...
                lbl.click(force=True)  # fallback (khi label không có "for")
