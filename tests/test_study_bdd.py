...
def practice_limit_info():
    with allure.step("Kiểm tra giới hạn luyện tập (manual)"):
        logger.info("⚙️ (Manual) Kiểm tra giới hạn luyện tập tối đa 5 lần.")
...
def theory_elements():
    with allure.step("Kiểm tra phần tử hiển thị trong lý thuyết (manual)"):
        logger.info("⚙️ Kiểm tra bộ đếm, nút in bài (manual).")
...
def qa_comment_section():
    with allure.step("Kiểm tra khu vực hỏi đáp và bình luận (manual)"):
        logger.info("⚙️ Kiểm tra khu vực hỏi đáp & bình luận (manual).")
...
def click_submit_without_answer(study_page):
    with allure.step("Người dùng click 'Nộp bài' khi chưa chọn đáp án"):
        ...
        logger.info("⚙️ Người dùng click 'Nộp bài' khi chưa chọn đáp án.")

@then("hệ thống hiển thị popup cảnh báo")
def verify_popup():
    with allure.step("Kiểm tra popup cảnh báo (manual)"):
        logger.info("⚙️ (Manual) Kiểm tra popup cảnh báo khi chưa chọn đáp án.")
